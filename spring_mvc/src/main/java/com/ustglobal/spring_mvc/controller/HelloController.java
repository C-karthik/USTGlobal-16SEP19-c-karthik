...
import com.ustglobal.spring_mvc.beans.Employee;

@Controller
public class HelloController<SimpleDateFormate> {
	...
	public String hi(@PathVariable("name") String name ,@PathVariable("id") int id,HttpServletRequest request) {
		...
		request.setAttribute("msg", "hi java how r u");
	...
	public String createCookie(ModelMap map ,HttpServletResponse response) {
		...
		Cookie cookie = new Cookie("name", "Bullet Siri");
	...
	@RequestMapping(path = "/form")   //controller class has doget method
	...
	@RequestMapping(path = "/form" , method = RequestMethod.POST)   //dopost method
